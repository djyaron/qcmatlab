classdef Controller < handle
   ...
   properties
      ...
      paramFile   % Directory to the parameterfile used for INDO
   end
   methods
      function obj = Controller(datapath, template, params, program, paramfile)
         if (nargin <5)
            paramfile = [];
         end
         ...
         obj.program = program;
         obj.paramFile = paramfile;
         
         % separate the paramNames, params, and inname values
         temp = cell(1,length(params));
         ...
         if length(temp) ~= 0
            ...
            for i=1:length(obj.iterations)
               ...
               if (paramfile)
                  obj.outputs{i} = obj.program(obj.dataPath, obj.template, tparam, paramfile);
               else
                  obj.outputs{i} = obj.program(obj.dataPath, obj.template, tparam);
               end
            end
         else
            obj.outputs{1} = obj.program(obj.dataPath, obj.template, []);
         end
      end
