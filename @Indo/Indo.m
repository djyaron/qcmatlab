classdef Indo < Base
    properties
        ...
        trans       % (25,2) the major transitions of the 25 states
    end
    ...
    methods
        ...
        function run(obj)
            ...
            elseif ~isempty(regexp(result,'ERROR','ONCE'))
                error(result(regexp(result,'ERROR','ONCE'):end))
            else
        ...
        function setTrans(obj,limit)
            if (nargin < 2)
                limit = 0.2;
            end
            obj.trans = cell(25,1);
            for iexc = 1:25
                Sn = (obj.wfsci(:,iexc).^2)>limit;
                obj.trans{iexc} = obj.ehsci(Sn,:);
            end
        end
    end
end